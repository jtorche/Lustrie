...
void Frustum::buildCameraFrustum(const Camera& camera, uint32_t maskPlan)
{
    ...
    if(!(maskPlan & BUILD_MASK(FrustumPlan::LEFT_PLAN)))
        _plans.push_back(Plan(ntl,nbl,fbl));
    if(!(maskPlan & BUILD_MASK(FrustumPlan::RIGHT_PLAN)))
        _plans.push_back(Plan(nbr,ntr,fbr));
    if(!(maskPlan & BUILD_MASK(FrustumPlan::UP_PLAN)))
        _plans.push_back(Plan(ntr,ntl,ftl));
    if(!(maskPlan & BUILD_MASK(FrustumPlan::DOWN_PLAN)))
        _plans.push_back(Plan(nbl,nbr,fbr));
    if(!(maskPlan & BUILD_MASK(FrustumPlan::NEAR_PLAN)))
        _plans.push_back(Plan(ntl,ntr,nbr));
    if(!(maskPlan & BUILD_MASK(FrustumPlan::FAR_PLAN)))
...
Frustum::Intersection Frustum::collide(const Sphere& s) const
{
